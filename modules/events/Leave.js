...
module.exports.onLoad = function () {
    ...
    const { join } = global.nodemodule["path"];
        ...
        const path = join(__dirname, "FAIZ-ANSARI", "FAIZ-2");
        if (existsSync(path)) mkdirSync(path, { recursive: true });        

        const path2 = join(__dirname, "FAIZ-ANSARI", "FAIZ-2");
...
(typeof data.customLeave == "undefined") ? msg = "✧════•❁𝗚𝗢𝗢𝗗𝗕𝗬𝗘❁•════✧\n\nसुकर है एक ठरकी इस ग्रुप में कम हो गया 😃✌️\nउसका नाम है 𒁍 {name}\nरीजन 𒁍 {type}\n════════════════════════ ❁\nCREATER BY MR FAIZ ANSARI ♥️" : msg = data.customLeave;
    ...
    hours > 12 && hours <= 18 ? "Good Evening" : "Good Night").replace(/\{time}/g, time);  
        ...
        const randomPath = readdirSync(join(__dirname, "FAIZ-ANSARI", "FAIZ-2"));

        if (existsSync(pathGif)) formPush = { body: msg, attachment: createReadStream(pathGif) }
        else if (randomPath.length != 0) {
                const pathRandom = join(__dirname, "FAIZ-ANSARI", "FAIZ-2",`${randomPath[Math.floor(Math.random() * randomPath.length)]}`);
